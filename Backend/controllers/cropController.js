...
export async function getCrops(req, res) {
  try {
    ...
    const result = await pool.query(
      `SELECT id, crop_name, variety, block_id, planting_date, expected_harvest_date, status, expected_yield, yield_unit, notes, created_at`
...
export async function addCrop(req, res) {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO crop_cycles (farm_id, block_id, crop_name, variety, planting_date, expected_harvest_date, season, expected_yield, yield_unit, notes, status)`
